-- ============================================================================
-- Test: User provisioning and default organization creation
-- ============================================================================

DO $$
DECLARE
    v_alice_id UUID := current_setting('test.alice_id')::UUID;
    v_user membership."user";
    v_org membership.organization;
    v_member_count BIGINT;
BEGIN
    RAISE DEBUG '→ Testing user upsert';

    SELECT * INTO STRICT v_user FROM membership."user" WHERE object_id = v_alice_id;

    IF v_user.email IS DISTINCT FROM 'alice@example.com' THEN
        RAISE EXCEPTION 'TEST FAILED: expected alice email, got %', v_user.email;
    END IF;
    IF v_user.email_verified IS DISTINCT FROM true THEN
        RAISE EXCEPTION 'TEST FAILED: alice should be email_verified';
    END IF;
    RAISE DEBUG '  ✓ User created with correct attributes';

    SELECT * INTO STRICT v_org
    FROM membership.organization
    WHERE owner_user_id = v_alice_id AND is_personal = true;

    IF v_org.name IS DISTINCT FROM 'Personal' THEN
        RAISE EXCEPTION 'TEST FAILED: expected Personal org, got %', v_org.name;
    END IF;
    RAISE DEBUG '  ✓ Personal organization created';

    SELECT count(*) INTO v_member_count
    FROM membership.organization_member
    WHERE organization_id = v_org.object_id AND user_id = v_alice_id AND status = 'active';

    IF v_member_count IS DISTINCT FROM 1 THEN
        RAISE EXCEPTION 'TEST FAILED: expected 1 active membership, got %', v_member_count;
    END IF;
    RAISE DEBUG '  ✓ User is active admin member of personal org';

    PERFORM membership.upsert_user('google', 'alice-001', 'alice@example.com', 'Alice Updated', true);
    SELECT * INTO STRICT v_user FROM membership."user" WHERE object_id = v_alice_id;
    IF v_user.display_name IS DISTINCT FROM 'Alice Updated' THEN
        RAISE EXCEPTION 'TEST FAILED: display_name not updated on re-upsert';
    END IF;
    RAISE DEBUG '  ✓ Re-upsert updates display_name';

    -- View-layer: vw_users shows correct role aggregation
    DECLARE
        v_view_roles text[];
        v_view_email text;
    BEGIN
        SELECT email, roles INTO v_view_email, v_view_roles
        FROM membership.vw_users WHERE object_id = v_alice_id;

        IF v_view_email IS DISTINCT FROM 'alice@example.com' THEN
            RAISE EXCEPTION 'TEST FAILED: vw_users email mismatch: %', v_view_email;
        END IF;
        IF v_view_roles IS DISTINCT FROM '{}' THEN
            RAISE EXCEPTION 'TEST FAILED: vw_users roles should be empty array for user with no roles, got %', v_view_roles;
        END IF;
        RAISE DEBUG '  ✓ vw_users shows correct attributes and empty roles array';
    END;

    RAISE DEBUG '✓ User upsert tests passed';
END $$;

DO $$
DECLARE
    v_customer_role TEXT := pg_temp.deployment_setting('database_customer_role');
BEGIN
    IF has_function_privilege(
        v_customer_role,
        'membership.upsert_user(TEXT,TEXT,TEXT,TEXT,BOOLEAN)',
        'EXECUTE'
    ) THEN
        RAISE EXCEPTION 'TEST FAILED: customer role must NOT have EXECUTE on upsert_user (identity overwrite risk)';
    END IF;
    RAISE DEBUG '  ✓ customer role denied EXECUTE on upsert_user';
END $$;

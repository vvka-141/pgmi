...
BEGIN
    ...
    v_alice_id := membership.upsert_user('google', 'alice-001', 'alice@example.com', 'Alice', true);
    PERFORM set_config('test.alice_id', v_alice_id::TEXT, true);

    v_bob_id := membership.upsert_user('github', 'bob-001', 'bob@example.com', 'Bob', true);

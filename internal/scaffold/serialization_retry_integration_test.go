...
// The MCP path has its own dispatcher above internal.mcp_dispatch. That lower
// layer re-raises the retryable class deliberately, but api.mcp_handle_request
// sat above it with a bare WHEN OTHERS and swallowed what the layer below
// preserved — the write vanished while the client saw -32603 and never learned
// to retry. This drives the same two-transaction conflict through MCP.
func TestAdvancedTemplate_MCPSerializationFailureRetryContract(t *testing.T) {
	connString := testhelpers.RequireDatabase(t)
	ctx := context.Background()

	testDB := "pgmi_test_mcp_retry_contract"
	deployAdvancedTemplate(t, connString, testDB)
	defer testhelpers.CleanupTestDB(t, connString, testDB)
	...
	if _, err := pool.Exec(ctx, `
		CREATE TABLE public.mcp_contended_counter (id int PRIMARY KEY, v int NOT NULL);
		INSERT INTO public.mcp_contended_counter VALUES (1, 0);
		GRANT ALL ON public.mcp_contended_counter TO PUBLIC;

		SELECT api.create_or_replace_mcp_handler(
			jsonb_build_object(
				'id', 'ffffffff-0179-4000-8000-000000000002',
				'type', 'tool',
				'name', 'mcp_contend_test',
				'description', 'Increments a contended row',
				'inputSchema', jsonb_build_object('type', 'object'),
				'requiresAuth', false
			),
			$body$
		BEGIN
			UPDATE public.mcp_contended_counter SET v = v + 1 WHERE id = 1;
	...
	`); err != nil {
		t.Fatalf("failed to set up the contended MCP tool: %v", err)
	}
	...
	const invoke = `SELECT (api.mcp_handle_request(
		'{"jsonrpc":"2.0","id":"c1","method":"tools/call","params":{"name":"mcp_contend_test","arguments":{}}}'::jsonb
	)).envelope`

	if _, err := winner.Exec(ctx, "BEGIN ISOLATION LEVEL REPEATABLE READ"); err != nil {
		t.Fatalf("winner BEGIN: %v", err)
	}
	var winnerEnvelope []byte
	if err := winner.QueryRow(ctx, invoke).Scan(&winnerEnvelope); err != nil {
		t.Fatalf("winner invoke: %v", err)
	}
	if bytes.Contains(winnerEnvelope, []byte(`"error"`)) {
		t.Fatalf("winner should have succeeded, got: %s", winnerEnvelope)
	}
	...
	go func() {
		var envelope []byte
		loserResult <- loser.QueryRow(ctx, invoke).Scan(&envelope)
	}()
	...
	err = <-loserResult
	if err == nil {
		t.Fatal("the losing MCP call must fail: mcp_handle_request swallowed the serialization " +
			"failure and returned an envelope, so the gateway never retries and the write is lost")
	}
	...
	if pgErr.Code != "40001" {
		t.Fatalf("expected SQLSTATE 40001 to reach the MCP client, got %s: %s", pgErr.Code, pgErr.Message)
	}
	...
	var retryEnvelope []byte
	if err := loser.QueryRow(ctx, invoke).Scan(&retryEnvelope); err != nil {
		t.Fatalf("retry on a fresh transaction should succeed, got: %v", err)
	}
	if _, err := loser.Exec(ctx, "COMMIT"); err != nil {
		t.Fatalf("loser retry COMMIT: %v", err)
	...
	var final int
	if err := pool.QueryRow(ctx, "SELECT v FROM public.mcp_contended_counter WHERE id = 1").Scan(&final); err != nil {
		t.Fatalf("read final counter: %v", err)
	}
	if final != 2 {
		t.Errorf("both increments must be durable after the retry; counter is %d, want 2 "+

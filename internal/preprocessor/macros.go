...
func NewMacroDetector() MacroDetector {
	...
	pattern := regexp.MustCompile(
		`(?:^|[^a-zA-Z0-9_])(?:pg_temp\.)?pgmi_(plan_)?test\s*\(\s*(?:'([^']*)'|NULL)?\s*\)`,
	)
...
func (d *macroDetector) Detect(sql string) []MacroCall {
	...
	for _, match := range matches {
		...
		if startPos < len(sql) {
			...
			// Check if first char is not part of the macro name
			if len(matchedText) > 0 && !isIdentChar(rune(matchedText[0])) && matchedText[0] != 'p' {
				startPos++
			}
		}
...
// isIdentChar returns true if r is a valid SQL identifier character.

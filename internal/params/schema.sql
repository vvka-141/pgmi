...
--   §pgmi_declare_param    - Parameter declaration with validation
--   §pgmi_get_param        - Parameter retrieval
...
-- §_pgmi_parameter ───────────────────────────────────────────────────────────
-- Populated by: Go CLI (--param k=v), enriched by pgmi_declare_param()
-- Used by: deploy.sql via pgmi_get_param()
...
-- §pgmi_declare_param ─────────────────────────────────────────────────────────
-- Called by: deploy.sql to declare expected parameters with type/default/required
-- Merges with: CLI-provided values (CLI wins if both exist)
-- Types: text, int, bigint, numeric, boolean, uuid, timestamp, timestamptz, name
...
BEGIN
    ...
    IF NOT v_exists THEN
        IF p_required AND p_default_value IS NULL THEN
            ...
            USING HINT = format('Available parameters: %s',
                (SELECT string_agg(key, ', ' ORDER BY key) FROM pg_temp._pgmi_parameter));
        END IF;

        v_value := p_default_value;

        -- Insert new parameter with all metadata
        INSERT INTO pg_temp._pgmi_parameter (key, value, type, required, default_value, description)
    ...
    ELSE
        ...
        UPDATE pg_temp._pgmi_parameter
        SET type = p_type,
            required = p_required,
...
-- §pgmi_get_param ─────────────────────────────────────────────────────────────
-- Returns parameter value or default; reads from session config set by declare_param

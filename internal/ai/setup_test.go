...
func TestParseManaged_CRLFNotEdited(t *testing.T) {
	f := generate(t)
	crlf := strings.ReplaceAll(f.Content, "\n", "\r\n")
	m := ParseManaged(crlf)
	if !m.HasStamp {
		t.Fatal("expected HasStamp after CRLF conversion")
	}
	if m.Edited() {
		t.Error("CRLF line endings should not be reported as edited")
	}
}

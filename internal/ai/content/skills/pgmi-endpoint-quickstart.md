...
| `minTransactionIsolation` | — (no floor) | Isolation floor (`read committed` / `repeatable read` / `serializable`). The caller must open the transaction at ≥ this level or the gateway returns 428 (`pgmi.transaction_isolation_too_weak`). |

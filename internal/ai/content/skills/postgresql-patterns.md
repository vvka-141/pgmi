...
**In the advanced template**: Use `api.error_response(sqlstate, message)` or `api.problem_response(status, title, detail)` for structured error handling (see `lib/api/07-helpers.sql`).

...
func TestResolveConnectionParams_ConflictDetection(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			envVars := &EnvVars{}
			_, _, err := ResolveConnectionParams(tt.connString, tt.granularFlags, nil, nil, envVars, nil)
...
func TestResolveConnectionParams_FromGranularFlags(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			config, maintenanceDB, err := ResolveConnectionParams("", tt.flags, nil, nil, tt.envVars, nil)
			if err != nil {
				t.Fatalf("unexpected error: %v", err)
			}
...
func TestResolveConnectionParams_InvalidPGPORT(t *testing.T) {
	...
	_, _, err := ResolveConnectionParams("", flags, nil, nil, envVars, nil)
	if err == nil {
...
func TestResolveConnectionParams_NilInputs(t *testing.T) {
	...
	config, maintenanceDB, err := ResolveConnectionParams("", nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestResolveConnectionParams_Precedence(t *testing.T) {
	...
	config, _, err := ResolveConnectionParams("", flags, nil, nil, envVars, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestResolveConnectionParams_ProjectConfig(t *testing.T) {
	...
	t.Run("project config used when no flags or env vars", func(t *testing.T) {
		cfg, _, err := ResolveConnectionParams("", &GranularConnFlags{}, nil, nil, &EnvVars{}, pc)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("flags override project config and env vars", func(t *testing.T) {
		flags := &GranularConnFlags{Host: "flaghost"}
		cfg, _, err := ResolveConnectionParams("", flags, nil, nil, &EnvVars{}, pc)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}

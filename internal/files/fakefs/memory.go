...
func (f *memoryFile) Info() filesystem.FileInfo { return f.info }
...
func (d *memoryDirectory) Walk(fn func(filesystem.File, error) error) error {
	...
	for _, entry := range entries {
		if slices.ContainsFunc(skipped, func(dir string) bool { return strings.HasPrefix(entry.absPath, dir+"/") }) {
			continue
		}
		if err := fn(entry, nil); err != nil {
			if errors.Is(err, fs.SkipDir) && entry.info.IsDir() {
				skipped = append(skipped, entry.absPath)
				continue
			}
			return err
		}
...
func (mfs *MemoryFileSystem) Open(openPath string) (filesystem.Directory, error) {
	...
	if exists {
		...
		// filesystem.Directory exists, return it
...
func (mfs *MemoryFileSystem) Stat(statPath string) (filesystem.FileInfo, error) {
	// Normalize to forward slashes (virtual filesystem convention)

...
│                                                                          │
│     These functions DON'T execute SQL—they INSERT into pgmi_plan        │
└────────────────────────────────────────────┬────────────────────────────┘
                                             │
                                             ▼
┌─────────────────────────────────────────────────────────────────────────┐
...
These views and functions are the stable API for deploy.sql. Use these instead of querying raw tables directly.
...
This view joins `pgmi_source` with `pgmi_source_metadata` and provides a clean interface for file access:
...
```sql
-- Deploy files in metadata-driven order
FOR v_file IN (
    SELECT path, content
...
FOR v_file IN (
    SELECT path FROM pg_temp.pgmi_plan_view
...
Files from `__test__/` or `__tests__/` directories are automatically organized here:
...
Generate standalone SQL test scripts that can run without pgmi.
...
These tables are the underlying storage for the session API. Users should generally use the views and functions above rather than querying these tables directly.

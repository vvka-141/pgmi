...
Worth stating plainly, because it indicts process rather than model: the project's hundred-plus SQL test files contained zero concurrency tests. Test-driven development — human or agent — cannot see these races, because nobody writes a red test for an interleaving they haven't imagined. The discipline has to be structural: *state-dependent mutations live in the kernel, and the write itself is guarded — by a row lock, a state predicate, or both.* Handlers probe only for things that produce different HTTP status codes, and treat even those probes as advisory — the kernel's guarded return value is the authority. Placement is doing the work in that sentence: a check and the state change it authorises have to be one guarded write, because anything evaluated a step earlier is evidence about a moment that has already passed — the general argument is [A Decision Belongs Where Its Authority Lives](https://alexeyevlampiev.github.io/locality-of-authority/).

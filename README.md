...
No SQL files loaded yet. Note that `deploy.sql` is never loaded into `pgmi_source`—it's the orchestrator that pgmi executes directly, not a deployable file.
...
### Zero-flag deployment

Without configuration, every deployment requires connection details:

```bash
pgmi deploy . --host localhost --port 5432 --username postgres --database myapp --param env=development
```

In CI/CD, you'd repeat this in every pipeline step. `pgmi.yaml` solves this—store your defaults once:

```yaml
# pgmi.yaml (generated by pgmi init)
connection:
  host: localhost
  port: 5432
  username: postgres
...
Now deployments become zero-flag:

```bash
pgmi deploy .                              # Uses pgmi.yaml defaults
pgmi deploy . --param env=production       # Override a single parameter
pgmi deploy . -d staging_db                # Override the database
```

CLI flags always take precedence: `CLI flags → env vars → pgmi.yaml → built-in defaults`. See [docs/CONFIGURATION.md](docs/CONFIGURATION.md) for the full reference.
...
SELECT pg_temp.pgmi_declare_param(
    ...
    p_type => 'text',
    p_required => true,
    p_description => 'Password for the application database role'
);
...
BEGIN
    ...
    -- Set application role password from parameter
    PERFORM pg_temp.pgmi_plan_do($$
        ALTER ROLE app_user WITH PASSWORD %L;
    $$, pg_temp.pgmi_get_param('app_password'));
...
**CLI usage:**
```bash
pgmi deploy . -d myapp --param app_password=SecurePass123
```

If `app_password` is missing, deployment fails immediately with a clear error—the parameter is required.
...
**Project-level configuration (recommended):** Add a `pgmi.yaml` to your project root with connection defaults and parameters. Generated automatically by `pgmi init`. See [docs/CONFIGURATION.md](docs/CONFIGURATION.md) for the full schema and precedence rules.

**Environment variable:** Set your database connection string using the `PGMI_CONNECTION_STRING` environment variable:
...
**Deployment Features:**
- Plan preview mode: run `deploy.sql` to build the execution plan, display scheduled commands, stop before execution. This shows what `deploy.sql` decided to schedule—pgmi never attempts rollback-based "dry run" since transaction control belongs to your SQL.
- Placeholder rendering: deterministic substitution with validation
- Error taxonomy for transient vs fatal errors (retry logic remains in `deploy.sql`)
